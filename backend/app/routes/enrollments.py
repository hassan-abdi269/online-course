...
def list_enrollments():
    ...
    return jsonify([{
        ...
        "course": {"id": enrollment.course.id, "title": enrollment.course.title, "image": enrollment.course.image} if enrollment.course else None,
...
def create_enrollment():
    ...
    student = db.get_or_404(User, data.get("student_id"))
...
def update_enrollment(enrollment_id):
    ...
    db.session.commit()
    return jsonify({"id": enrollment.id, "progress": enrollment.progress, "status": enrollment.status})
